...
Now on to my second question: How does viewing of shows change across the four reports in the dataset? This is a fairly broad question so I'm going to
...
My first question concerns how views change longitudinally across the four reporting periods. I'm going to examine this from three angles: 

* First, I want to select a couple popular shows and see how their views have flucuated over the four reports
* Second, I want to limit the question to limited series and see how those one-off shows perform over time
* Third, I want to lump shows together by their season number and see if perhaps later seasons experience higher dropoff than earlier seasons

#### Popular Shows

-Total Show views X report date: Sum views of all show seasons by each report period
-Total Show views X report date and contains_new_season: Sum views of all show seasons by each report period and whether contains_new_season = TRUE
-Show views X report date, season-for single/two shows: Views for each season of each show
-Show Views X season
...
multi_report_shows |> 
  ...
  pivot_wider(id_cols = -total_views, names_from = series, values_from = subsequent_report_dropoff) |> 
  gt() |> 
  format_table_header() |> 
...
calc_table_data <- function(data, grouping_var){
...
build_formatted_table <- function(data, grouping_var){
  group <- enquo(grouping_var)
  ...
  # create a set of named vectors containing the logic for formatting table
  formatting_flag <- data |> 
    summarise(flags = list(contains_new_season), .by = !!group) |> 
    deframe()
    
  
  # build final formatted table
    reduce(
      names(formatting_flag),
      .init = base_table,
      function(gt_obj, current_item){
        ...
        tab_style(
          ...
          locations = cells_body(
            ...
            rows = formatting_flag[[current_item]]
          )
        )
      }
    ) |>
...
multi_report_shows |> 
  ...
  # filter(startsWith(title,'You:') | series %in% c('Stranger Things', 'Breaking Bad', 'Seinfeld'))   |> 
  calc_table_data(season) |> 
  build_formatted_table(season)
