...
two parts. Part one will look at the overall level of viewership and part two will look at viewership for individual series and seasons.
Let's get started!
...
shows in the dataset and grouped them by their report[^most-viewed-note]. @tbl-two is the result.
...
from this small group of shows--there are ~500 million more 2025 views in the top 100 than any other report.
I'm not going to dive into this, but it's something to keep in mind.
...
seasonal_all_shows <- shows_cleaned |> 
  ...
  calc_table_data(season, report_date) |> 
...
@tbl-four shows similar traits to season one from @tbl-three: there are a lot of shows in the data and the views for 
each season are relatively stable. In contrast, @tbl-five demonstrates how impactful new seasons are, but suffers from the 
later seasons problem mentioned above: each season in @tbl-five has so few shows that a single show having a big viewership
disproportionately impacts the rest of the data. 

There are ways we could try to lessen single show impact, but these huge summation tables aren't scratching my data itch. Looking at shows at this level of aggregation washes out so much detail that it's difficult to answer interesting questions like 'how quickly do shows lose their audience' or 
...
we don't have access to) because at the level of the entire dataset it's somewhat meaningless to ask 'how much viewership a show lose between season, on average'. There isn't really 
...
shows_cleaned |> 
  ...
  summarise(total_views = sum(views, na.rm = TRUE),
            total_shows = n()) |> 
  mutate(perc_change = (total_views - lag(total_views, 1))/lag(total_views, 1) *100) |> 
  pivot_wider(-total_views, names_from = report_date, values_from = perc_change)

shows_cleaned |> 
  ungroup() |> 
  filter(season %in% paste0("Season ", 1:7)) |>
  # filter(series %in% c("Virgin River", 'Seinfeld', 'Arrested Development')) |>
  calc_table_data(report_date, season) |> 
  build_formatted_table(add_conditional_highlights = FALSE, report_date, season)


shows_cleaned |> 
  ungroup() |> 
  calc_table_data(report_date) |> 
  build_formatted_table(add_conditional_highlights = FALSE, report_date)
```
